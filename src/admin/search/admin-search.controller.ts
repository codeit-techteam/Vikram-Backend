...
export class AdminSearchController {
  ...
  @Get()
  @ApiOperation({ summary: 'Global admin search across customers, products, orders, memberships, bulk' })
  @ApiQuery({ name: 'q', required: true, description: 'Search query (min 2 chars)' })
  @ApiQuery({ name: 'limit', required: false, description: 'Results per entity type (default: 10)' })
  async search(@Query('q') q: string, @Query('limit') limit?: number) {
    const data = await this.searchService.globalSearch(q, limit ? Number(limit) : 10);
