...
import { CreateTestimonialDto, UpdateTestimonialDto, TestimonialQueryDto } from './dto/admin-testimonials.dto';
...
export class AdminTestimonialsController {
  ...
  @Post()
  @AdminRoles(...ROLE_GROUPS.SUPER_ADMIN_ONLY)
  @ApiOperation({ summary: 'Create testimonial (VIDEO, IMAGE, or text review)' })
  async create(@Body() dto: CreateTestimonialDto, @CurrentAdmin() admin: AuthenticatedAdmin) {
    const data = await this.testimonialsService.create(dto);
    await this.auditService.log({ adminUserId: admin.id, adminEmail: admin.email, action: 'CREATE', resource: 'Testimonial', resourceId: data.id, newValue: dto });
  ...
  async reorder(@Body() body: { items: Array<{ id: string; sortOrder: number }> }) {
  ...
  async update(@Param('id') id: string, @Body() dto: UpdateTestimonialDto, @CurrentAdmin() admin: AuthenticatedAdmin) {
    const data = await this.testimonialsService.update(id, dto);
    await this.auditService.log({ adminUserId: admin.id, adminEmail: admin.email, action: 'UPDATE', resource: 'Testimonial', resourceId: id, newValue: dto });
    return { success: true, message: 'Testimonial updated', data };
  }

  @Patch(':id/publish')
  @AdminRoles(...ROLE_GROUPS.SUPER_ADMIN_ONLY)
  @ApiOperation({ summary: 'Publish testimonial (auto-consumed by Customer App Home API)' })
  async publish(@Param('id') id: string, @CurrentAdmin() admin: AuthenticatedAdmin) {
    ...
    await this.auditService.log({ adminUserId: admin.id, adminEmail: admin.email, action: 'PUBLISH', resource: 'Testimonial', resourceId: id });
  ...
  async unpublish(@Param('id') id: string, @CurrentAdmin() admin: AuthenticatedAdmin) {
    ...
    await this.auditService.log({ adminUserId: admin.id, adminEmail: admin.email, action: 'UNPUBLISH', resource: 'Testimonial', resourceId: id });
  ...
  async remove(@Param('id') id: string, @CurrentAdmin() admin: AuthenticatedAdmin) {
    ...
    await this.auditService.log({ adminUserId: admin.id, adminEmail: admin.email, action: 'DELETE', resource: 'Testimonial', resourceId: id });
