...
export const MATERIAL_TRANSPORT_PROFILES: Record<
  string,
...
> = {
  ...
  RMC: {
    ...
    reason:
      'RMC requires a transit mixer truck and cannot use Bike, E-Loader, or Pickup.',
  },
  BRICKS: {
    ...
    reason:
      'Bricks are not bike-compatible; bulk brick loads require Pickup or a heavy vehicle.',
  },
  CEMENT: {
    ...
    reason:
      'Cement bags exceed typical Bike payload; vehicle is selected from total bag weight.',
  },
  STEEL: {
    logisticsType: 'STEEL',
    allowedVehicleTypes: HEAVY,
  ...
  WATERPROOFING: {
    ...
    unloadingKgPerMinute: 70,
    reason:
      'Packaged waterproofing can use Bike or E-Loader when payload allows.',
  },
  ADHESIVE: {
  ...
  WALL_PUTTY: {
    ...
    reason:
      'Wall putty bags are packaged; vehicle is selected from total bag weight.',
  },
  QUICK_REPAIR: {
  ...
  LIGHT_MATERIAL: {
    ...
    unloadingKgPerMinute: 70,
    reason:
      'Lightweight packaged goods may use Bike when total load is within payload.',
  },
  PARCEL: {
...
export function resolveWeightPerUnitKg(input: {
  ...
  kg: number | null;
  source: 'product' | 'unit' | 'profile' | 'density' | null;
} {
