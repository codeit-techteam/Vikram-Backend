...
describe('delivery-slot.logic', () => {
  it('clips expired same-day windows using lead time', () => {
    const hours = resolveHubHours({ openMinutes: 9 * 60, closeMinutes: 21 * 60 });
    ...
    expect(windows.today.every((slot) => slot.startMinutes >= 18 * 60)).toBe(true);
    expect(windows.tomorrow.length).toBeGreaterThan(0);
  });

  it('uses a longer RMC lead time', () => {
    expect(resolveLeadMinutes({ isRmc: true, etaMinMinutes: 40 })).toBeGreaterThanOrEqual(90);
    expect(resolveLeadMinutes({ isRmc: false, etaMinMinutes: 40 })).toBe(40);
  });

  it('sanitizes delivery remarks', () => {
    expect(sanitizeDeliveryRemark('  Call before arriving <script>x</script> ')).toBe(
      'Call before arriving x',
    );
